...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('Raghad Star', tokenId, {from: owner});

    //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
    assert.equal(await instance.name(), 'Raghad Star')
    assert.equal(await instance.symbol(), 'RS')
});
...
it('lets a user transfer a star', async() => {
    ...
    // 1. create a Star with different tokenId
    await instance.createStar('New Star', 128, {from: user1});
    // 2. use the transferStar function implemented in the Smart Contract
    await instance.transferStar(user2, 128, {from: user1});
    // 3. Verify the star owner changed.
    assert.equal(await instance.ownerOf(128), user2);
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 384;
    // 1. create a Star with different tokenId
    await instance.createStar('Star with different tokenId', tokenId, {from: user1});
